  ...
  return <div className="p-4 sm:p-5 space-y-4">
    ...
    {tab==='tasks'&&<>
      <GlassCard className="p-4 space-y-2">
        ...
        <button disabled={saving} onClick={save} className="w-full py-2.5 rounded-lg bg-gradient-to-r from-indigo-600 to-teal-500 text-white text-[10px] font-black uppercase disabled:opacity-50">{saving?(editing?'Saving...':'Creating...'):(editing?'Save Task Changes':'Create Task')}</button>
      </GlassCard>
      ...
      <div className="space-y-2">
        ...
        {data.tasks.length===0 ? <GlassCard className="p-5 text-center text-[10px] text-slate-500">No tasks have been created yet.</GlassCard> : data.tasks.map((t:any)=><GlassCard key={t.id} className="p-4 space-y-3">
          <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-xs font-bold text-white">{t.title}</div><div className="text-[9px] text-slate-400 mt-1">₦{Number(t.rewardamount??t.rewardAmount??0).toLocaleString()} • {t.verificationtype||t.verificationType||'timer'}</div><div className="text-[10px] text-slate-500 mt-1 break-all">{t.description}</div>{(t.actionurl||t.actionUrl)&&<div className="text-[9px] text-cyan-400 mt-2 break-all">{t.actionurl||t.actionUrl}</div>}</div><span className={`shrink-0 text-[8px] px-2 py-1 rounded-full ${Number(t.enabled)===1?'bg-emerald-500/10 text-emerald-300':'bg-slate-500/10 text-slate-400'}`}>{Number(t.enabled)===1?'ACTIVE':'SUSPENDED'}</span></div>
          <div className="flex gap-2"><button onClick={()=>action(`/api/admin/nivo/tasks/${t.id}`,'PATCH',{enabled:!(Number(t.enabled)===1)})} className="flex-1 py-2 rounded-lg bg-white/5 text-slate-300 text-[9px] font-bold">{Number(t.enabled)===1?'Suspend Task':'Activate Task'}</button>{(t.actionurl||t.actionUrl)&&<a href={t.actionurl||t.actionUrl} target="_blank" rel="noreferrer" className="p-2 rounded-lg bg-cyan-500/10 text-cyan-300"><ExternalLink className="w-3.5 h-3.5"/></a>}<button onClick={()=>openEdit(t)} className="p-2 rounded-lg bg-white/5 text-slate-300"><Edit2 className="w-3.5 h-3.5"/></button><button onClick={()=>deleteTask(t)} className="p-2 rounded-lg bg-rose-500/10 text-rose-300"><Trash2 className="w-3.5 h-3.5"/></button></div>
        </GlassCard>)}
      </div>
    </>}
