...
export default function NivoFeaturesPanel({ user, token, onToast, initialTab = 'tasks' }: Props) {
  ...
  const activate = async () => {
    ...
    try {
      ...
      onToast?.('Activation completed successfully.', 'success');
  ...
  return <div className="p-4 sm:p-5 space-y-4 animate-[fadeIn_0.2s_ease-out] w-full max-w-full min-w-0">
    ...
    {tab === 'tasks' && <div className="space-y-3">{tasks.length === 0 && <GlassCard className="p-5 text-center text-xs text-slate-400">No tasks are available right now.</GlassCard>}{tasks.map(task => {
      ...
      return <GlassCard key={task.id} className="p-4 space-y-3">
        <div className="flex items-start gap-3"><div className="w-9 h-9 rounded-xl bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 flex items-center justify-center shrink-0"><Gift className="w-4 h-4" /></div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><h4 className="text-xs font-bold text-white">{task.title}</h4><span className="text-[10px] font-black text-teal-300">₦{Number(task.rewardAmount || 0).toLocaleString()}</span></div><p className="text-[10px] text-slate-400 mt-1 leading-relaxed">{task.description}</p></div></div>
        ...
        {task.status === 'approved' || task.status === 'claimed' ? <div className="text-[9px] text-emerald-300 bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-2">Reward approved</div> : <>
          ...
          <div className="flex gap-2"><button disabled={busy === task.id} onClick={() => startTask(task)} className="flex-1 py-2.5 rounded-lg bg-indigo-500/15 border border-indigo-500/20 text-indigo-200 text-[9px] font-bold uppercase">{busy === task.id ? 'Starting...' : 'Start'}</button><button disabled={busy === task.id} onClick={() => submitTask(task)} className="flex-1 py-2.5 rounded-lg bg-teal-500/15 border border-teal-500/20 text-teal-200 text-[9px] font-bold uppercase">{remaining ? `${remaining}s` : 'Submit'}</button></div>
        </>}
      </GlassCard>;
    })}</div>}
    ...
    {tab === 'activation' && <GlassCard className="p-5 space-y-4"><div className="text-center"><div className="w-12 h-12 rounded-2xl bg-teal-500/10 text-teal-300 border border-teal-500/20 flex items-center justify-center mx-auto"><ShieldCheck className="w-6 h-6" /></div><h4 className="text-sm font-bold text-white mt-3">Account Activation</h4><p className="text-[10px] text-slate-400 mt-1">Activate your account to unlock the full rewards wallet.</p></div>{activation?.activated ? <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-center text-xs font-bold">Account activated</div> : <><div className="flex justify-between text-xs"><span className="text-slate-400">Activation fee</span><span className="font-bold text-white">₦{Number(activation?.fee || 520).toLocaleString()}</span></div><button disabled={busy === 'activation'} onClick={activate} className="w-full py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-teal-500 text-white text-xs font-black uppercase">{busy === 'activation' ? 'Processing...' : 'Activate Account'}</button></>}</GlassCard>}
